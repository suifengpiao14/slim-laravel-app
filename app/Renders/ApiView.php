...
class ApiView
{
     ...
     * @param array|string      $data       Associative array of data to be returned
     * @param int               $status     HTTP status code
     * @param array             $addHeaders Associative array of header to be added
     *
     * @return ResponseInterface
     */
    public function render(Request $request, ResponseInterface $response, $data = 'ok', $status = 200, $addHeaders = [])
    {
        $mediaType = $this->determineMediaType($request);
        is_string($data) and $data = ['message' => $data];
